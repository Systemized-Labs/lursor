...
export function MarkdownRenderer({ children, className }: MarkdownRendererProps) {
  return (
    <div
      className={cn(
        'prose prose-sm dark:prose-invert min-w-0 max-w-full break-words',
        // Text
        '[&_p]:text-foreground [&_p]:my-1 [&_p]:leading-relaxed',
        '[&_li]:text-foreground',
        '[&_strong]:text-foreground',
        '[&_em]:text-foreground',
        '[&_blockquote]:text-muted-foreground [&_blockquote]:border-l-2 [&_blockquote]:border-border [&_blockquote]:pl-3',
        '[&_h1]:text-foreground [&_h2]:text-foreground [&_h3]:text-foreground [&_h4]:text-foreground',
        // Inline code
        '[&_:not(pre)>code]:text-foreground [&_:not(pre)>code]:bg-muted [&_:not(pre)>code]:px-1.5 [&_:not(pre)>code]:py-0.5 [&_:not(pre)>code]:rounded [&_:not(pre)>code]:text-xs [&_:not(pre)>code]:font-mono',
        ...
        '[&_td]:text-foreground [&_td]:px-3 [&_td]:py-2 [&_td]:border [&_td]:border-border',
        '[&_tr:nth-child(even)_td]:bg-muted/20',
        // Links
        '[&_a]:text-primary [&_a]:underline [&_a]:underline-offset-2',
        // Lists (explicit markers — no typography plugin / preflight resets them)
