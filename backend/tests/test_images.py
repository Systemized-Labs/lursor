...
async def test_submit_returns_before_the_image_and_then_completes(
    ...
    r = await client.post(
        f"/laios/connections/{cid}/images",
        json={
            "model": "z-image-turbo",
    ...
    r = await client.get(f"/laios/connections/{cid}/images/{run['id']}")
    ...
    r = await client.get(f"/laios/connections/{cid}/images/{run['id']}/content")
    assert r.status_code == 200, r.text
    assert r.content == JPEG
    assert r.headers["content-type"] == "image/jpeg"

    listed = (await client.get(f"/laios/connections/{cid}/images")).json()
...
async def test_response_format_and_n_are_forced(client: AsyncClient, monkeypatch):
    ...
    column cannot hold.
    """
    conn = await _make_connection(client, name="forced", base_url="http://f:7420")
    cid = conn["id"]
    captured: dict = {}
    _patch_gateway(monkeypatch, _generations(captured))
    ...
    stored = (await client.get(f"/laios/connections/{cid}/images/{run['id']}")).json()
...
async def test_engine_validation_message_lands_on_the_row(
    ...
    failed = (await client.get(f"/laios/connections/{cid}/images/{run['id']}")).json()
...
async def test_orphaned_running_row_is_reaped(client: AsyncClient, monkeypatch):
    ...
    """
    conn = await _make_connection(client, name="orphan", base_url="http://o:7420")
    cid = conn["id"]
    captured: dict = {}
...
async def test_url_response_falls_back_to_the_content_endpoint(
    ...
    """
    conn = await _make_connection(client, name="urlish", base_url="http://u:7420")
    cid = conn["id"]
    seen: dict = {}
    ...
    done = (await client.get(f"/laios/connections/{cid}/images/{run['id']}")).json()
...
async def test_submit_requires_a_model_and_a_prompt(client: AsyncClient):
    ...
    r = await client.post(f"/laios/connections/{cid}/images", json={"prompt": "x"})
    assert r.status_code == 400
    assert "model is required" in r.json()["detail"]

    r = await client.post(
        f"/laios/connections/{cid}/images", json={"model": "z-image-turbo"}
    )
    assert r.status_code == 400
    assert "prompt is required" in r.json()["detail"]

    assert (await client.get(f"/laios/connections/{cid}/images")).json() == []
...
async def test_delete_removes_the_run(client: AsyncClient, monkeypatch):
    ...
    r = await client.delete(f"/laios/connections/{cid}/images/{run['id']}")
    assert r.status_code == 200, r.text
    assert (await client.get(f"/laios/connections/{cid}/images")).json() == []
    assert (
        await client.get(f"/laios/connections/{cid}/images/{run['id']}")
    ).status_code == 404
...
async def test_content_is_404_while_still_running(client: AsyncClient, monkeypatch):
    ...
    conn = await _make_connection(client, name="early", base_url="http://e:7420")
    cid = conn["id"]
...
async def test_runs_are_scoped_to_their_connection(client: AsyncClient, monkeypatch):
    """One box's history is not another's — the list is keyed by connection."""
    ...
    run = (
        await client.post(
            f"/laios/connections/{a['id']}/images",
            json={"model": "z-image-turbo", "prompt": "only on a"},
        )
    ).json()
    await _settle(run["id"])

    assert len((await client.get(f"/laios/connections/{a['id']}/images")).json()) == 1
    assert (await client.get(f"/laios/connections/{b['id']}/images")).json() == []
    # And a run cannot be read through the wrong connection.
    assert (
